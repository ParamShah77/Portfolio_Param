/*
 * The WebGL half of the hero.
 *
 * Split out of Hero3D.jsx and loaded lazily: three.js, @react-three/fiber and
 * drei together are the bulk of the JavaScript on this site, and none of it is
 * needed to paint the name, which is what the visitor actually reads first.
 */
...
───────────────────────────────────────────── */
function Avatar({ mousePos, reducedMotion }) {
  ...
  useFrame((state) => {
    ...
    // Leave the avatar in its rest pose rather than tracking, blinking, or waving.
...
───────────────────────────────────────────── */
function FloatingOrb({ pos, size, speed, color, reducedMotion }) {
  const ref = useRef();
  useFrame(({ clock }) => {
...
function ResponsiveCamera() {
  ...
  useFrame(() => {
    ...
    // pattern — the renderer reads it each frame and it must not trigger a
    // React render. The immutability rule can't see that.
...
/* ─────────────────────────────────────────────
   Scene wrapper
───────────────────────────────────────────── */
function Scene({ mousePos, reducedMotion }) {
  return (
    <>
      <ResponsiveCamera />
      {/*
        Lighting. Deliberately no <Environment preset="…" />: drei downloads
        that HDR from raw.githack.com at runtime, which made the hero's
        appearance depend on a third-party CDN staying up. Every material here
        is metalness 0, so an env map contributed almost nothing anyway — a
        hemisphere light covers the soft ambient fill it was providing.
      */}
      <ambientLight intensity={0.65} />
      <hemisphereLight args={["#ffd9b3", "#1a1120", 0.55]} />
      ...
      {/* Background orbs */}
      <FloatingOrb pos={[-2.8, 1.2, -2]} size={0.18} speed={0.5} color="#f97316" reducedMotion={reducedMotion} />
      <FloatingOrb pos={[2.6, -0.8, -3]} size={0.24} speed={0.38} color="#fb923c" reducedMotion={reducedMotion} />
      <FloatingOrb pos={[1.4, 2.2, -4]} size={0.13} speed={0.72} color="#f97316" reducedMotion={reducedMotion} />
      <FloatingOrb pos={[-1.8, -1.4, -2]} size={0.1} speed={0.6} color="#fdba74" reducedMotion={reducedMotion} />
      ...
      >
        <Avatar mousePos={mousePos} reducedMotion={reducedMotion} />
      </Float>
    </>
  );
}
  ...
  return (
    ...
    >
      <Suspense fallback={null}>
        <Scene mousePos={mousePos} reducedMotion={reducedMotion} />
      </Suspense>
    </Canvas>
  );
}
